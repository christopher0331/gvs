...
class FeaturedProject1 extends Component {
  constructor(props) {
    super(props);
    ...
    return (
      <div>
        ...
        <div className="imageSection">
          <h2 className="featuredTitle">Project {projectNumber}</h2>
          ...
          <div>
            <div className="secondaryCarousel">
              {this.state.allImages.map((image, index) => (
                <img key={index} onClick={() => this.changePrimary(image)} className="secondaryImages" src={image} alt={image} />
              ))}
            </div>
          </div>
        </div>
